...
const schema = z.object({
  content: z.string().trim().min(3, "Qeyd ən azı 3 simvol olmalıdır").max(2000),
...
function JournalPage() {
  ...
  return (
    <Page>
      <PageHeader
        kicker="Tranzit jurnalı"
        title="Günlərini səmavi ritmlə izlə"
        subtitle="Əhvalını və hadisələri qeyd et, zamanla planet tranzitlərinin təsirini gör."
      />
      ...
      <div className="grid lg:grid-cols-12 gap-6">
        ...
        >
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label htmlFor="entry_date" className="block text-xs text-mist mb-1.5">Tarix</label>
            ...
            <div>
              <span className="block text-xs text-mist mb-1.5">Əhval</span>
              <div className="flex gap-1">
                {MOODS.map((m, i) => (
                  <button key={m} type="button" onClick={() => setForm({ ...form, mood: i + 1 })}
                    aria-label={`Əhval ${i + 1}`}
          ...
          <div>
            <label htmlFor="title" className="block text-xs text-mist mb-1.5">Başlıq</label>
            <input id="title" value={form.title} maxLength={120}
              onChange={(e) => setForm({ ...form, title: e.target.value })}
              className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-gold/50" />
          </div>
          <div>
            <label htmlFor="content" className="block text-xs text-mist mb-1.5">Bu gün nə oldu?</label>
          ...
          <div>
            <label htmlFor="transit_note" className="block text-xs text-mist mb-1.5">Tranzit qeydi</label>
            <input id="transit_note" value={form.transit_note} maxLength={300}
              placeholder="məs. Ay Əqrəbdə, Merkuri retro"
          ...
          <button type="submit" disabled={add.isPending}
            ...
            {add.isPending ? "Yazılır…" : "Qeyd əlavə et"}
          </button>
        </form>

        <div className="lg:col-span-7 space-y-4">
          <div className="rounded-2xl bg-celestial-card/60 border border-white/5 p-5 flex items-center justify-between">
            <span className="text-mist text-sm">Ümumi qeyd: {entries?.length ?? 0}</span>
            <span className="text-mist text-sm">Orta əhval: <span className="text-goldsoft">{avgMood}</span> / 5</span>
          </div>

          {entries?.length === 0 && <p className="text-mist">Hələ qeyd yoxdur.</p>}
            ...
            <article key={e.id} className="rounded-2xl bg-celestial-card/60 border border-white/5 p-5">
              <div className="flex items-start justify-between gap-4">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button type="button" aria-label="Sil" onClick={() => remove.mutate(e.id)}
