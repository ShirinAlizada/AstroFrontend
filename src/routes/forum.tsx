...
export const Route = createFileRoute("/forum")({
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "Forum — Ruh Astrolojiya" },
...
const CATEGORIES = ["ümumi", "tranzitlər", "natal xəritə", "sinastriya", "sual-cavab"];

const schema = z.object({
  title: z.string().trim().min(5, "Başlıq ən azı 5 simvol").max(140),
  body: z.string().trim().min(10, "Mətn ən azı 10 simvol").max(3000),
});
...
function ForumPage() {
  ...
  const create = useMutation({
    ...
    onSuccess: () => {
      toast.success("Mövzu paylaşıldı");
  ...
  return (
    <Page>
      <PageHeader
        kicker="İcma"
        title="Forum"
        subtitle="Tranzitləri müzakirə et, xəritə oxunuşları paylaş, suallarına cavab al."
      />
      ...
      <div className="grid lg:grid-cols-12 gap-6">
        <div className="lg:col-span-7 space-y-4">
          {visible?.length === 0 && <p className="text-mist">Bu kateqoriyada mövzu yoxdur.</p>}
          {visible?.map((t) => (
            <Link key={t.id} to="/forum/$topicId" params={{ topicId: t.id }}
              className="block rounded-2xl bg-celestial-card/60 border border-white/5 p-5 hover:border-gold/30 transition">
              <div className="flex items-center gap-3 text-xs text-mist">
                <span className="px-2.5 py-1 rounded-full border border-violet/30 text-violet">{t.category}</span>
                <span>{new Date(t.created_at).toLocaleDateString("az-AZ")}</span>
              </div>
              <h2 className="font-display text-2xl mt-2">{t.title}</h2>
              <p className="text-sm text-white/75 mt-1 line-clamp-2">{t.body}</p>
              <p className="text-xs text-mist mt-3">{t.author_name}</p>
            </Link>
          ))}
        </div>

        <aside className="lg:col-span-5 h-fit rounded-2xl bg-celestial-card/60 border border-white/5 p-6">
          <h2 className="font-display text-2xl mb-4">Yeni mövzu</h2>
          {user ? (
            <form onSubmit={(e) => { e.preventDefault(); create.mutate(); }} className="space-y-4">
              <div>
                <label htmlFor="cat" className="block text-xs text-mist mb-1.5">Kateqoriya</label>
              ...
              <div>
                <label htmlFor="t-title" className="block text-xs text-mist mb-1.5">Başlıq</label>
                <input id="t-title" value={form.title} maxLength={140}
                  onChange={(e) => setForm({ ...form, title: e.target.value })}
                  className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-gold/50" />
              </div>
              <div>
                <label htmlFor="t-body" className="block text-xs text-mist mb-1.5">Mətn</label>
            ...
            <div className="text-sm text-mist">
              Mövzu açmaq üçün{" "}
              <Link to="/auth" className="text-goldsoft hover:text-gold">daxil ol</Link>.
            </div>
          )}
        </aside>
      </div>
